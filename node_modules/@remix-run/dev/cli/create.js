...
async function validateNewProjectPath(input) {
  ...
  if ((await fse__default["default"].pathExists(projectDir)) && (await fse__default["default"].stat(projectDir)).isDirectory()) {
    if ((await fse__default["default"].readdir(projectDir)).length > 0) {
      throw Error("🚨 The current directory must be empty to create a new project. Please " + "clear the contents of the directory or choose a different path.");
    } else {
      throw Error("🚨 The directory provided already exists. Please try again with a " + "different directory.");
    }
  }
}
...
function isRemixTemplate(input) {
  return ["remix", "express", "arc", "fly", "netlify", "vercel", "cloudflare-pages", "cloudflare-workers"].includes(input);
}
    ...
    case "remoteTarball":
      {
        ...
        try {
          let response = await fetch__default["default"](input, {
            method: "HEAD"
          });
          spinner.stop();
    ...
    case "template":
      {
        ...
        try {
          let response = await fetch__default["default"](templateUrl, {
            method: "HEAD"
          });
          spinner.stop();
